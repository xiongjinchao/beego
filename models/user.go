...
//根据昵称查用户
func  QueryUserByName(name string) (v *User, err error) {
	o := orm.NewOrm()
	v = &User{Name: name}
...
//根据邮箱查用户
func QueryUserByEmail(email string) (v *User, err error) {
	o := orm.NewOrm()
	v = &User{Email: email}
...
func Register(name string, password string) (v *User, err error) {
	o := orm.NewOrm()
	v = &User{Name: name, Password: password}
	if err = o.QueryTable(new(User)).Filter("Name", name).Filter("Password", password).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
